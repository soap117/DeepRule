...
from .utils import random_crop_xy, draw_gaussian, gaussian_radius
...
def kp_detection(db, k_ind, binary_file, data_aug, debug):
    ...
    # allocating memory
    images      = np.zeros((batch_size, 5, input_size[0], input_size[1]), dtype=np.float32)
    ...
    for b_ind in range(batch_size):
        ...
        image_file = re.findall(".*/images/(.*)", image_file)[0]
        #print(image_file)
        npimg = numpy.frombuffer(binary_file[image_file], numpy.uint8)
        image = cv2.imdecode(npimg, cv2.IMREAD_COLOR)
        image_xy = np.zeros((image.shape[0], image.shape[1], 2), dtype=np.float32)
        x_mark = np.arange(image.shape[1], dtype=np.float32)/image.shape[1]
        for i in range(image.shape[0]):
            image_xy[i, :, 0] = x_mark
        y_mark = np.arange(image.shape[0], dtype=np.float32)/image.shape[0]
        ...
        if not debug and rand_crop:
            image, detections, image_xy = random_crop_xy(image, image_xy, detections, rand_scales, input_size, border=border)
        else:
        ...
        image_xy = cv2.resize(image_xy, (new_width, new_height))
        ...
        images[b_ind][3:5] = image_xy.transpose((2, 0, 1))
...
def sample_data(db, k_ind, binary_file, data_aug=True, debug=False):
    return globals()[system_configs.sampling_function](db, k_ind, binary_file, data_aug, debug)
