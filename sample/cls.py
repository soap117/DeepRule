...
from .utils import random_crop_cls, draw_gaussian, gaussian_radius
...
def kp_detection(db, k_ind, data_aug, debug):
    ...
    clss = np.zeros((batch_size,), dtype=np.int64)
    offsets = np.zeros((batch_size,), dtype=np.float32)
    ...
    for b_ind in range(batch_size):
        ...
        cat_image, offset_image = db.image_info(db_ind)
        clss[b_ind] = cat_image
        #print(image_file)
        ...
        if not debug and rand_crop:
            image, detections, offset_image = random_crop_cls(image, detections, offset_image, rand_scales, input_size, border=border)
        else:
            image, detections = _full_image_crop(image, detections)
        offsets[b_ind] = (offset_image / 100) - 1
        #print("Image_size")
        #print(image.shape)
    ...
    return {
        ...
        "ys": [tl_heatmaps, br_heatmaps, tag_masks, tl_regrs, br_regrs, clss, offsets]
    }, k_ind
