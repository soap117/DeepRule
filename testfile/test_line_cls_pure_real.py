...
def kp_detection(db, nnet, result_dir, debug=False, decode_func=kp_decode):
    image_info_path = os.path.join(result_dir, "link_pre_real.json")
    ...
    if True:
        ...
        for ind in tqdm(range(0, num_images), ncols=80, desc="locating kps"):
            try:
                ...
                predictions, flag = decode_func(nnet, [images, tags, weights, tag_masks], K, ae_threshold=ae_threshold, kernel=nms_kernel)
                print(predictions)
