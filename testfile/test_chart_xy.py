...
def kp_decode(nnet, images, K, ae_threshold=0.5, kernel=3):
    with torch.no_grad():
        try:
            detections, detections_tl, detections_br = nnet.test([images], ae_threshold=ae_threshold, K=K,
                                                                 kernel=kernel)
    ...
    if True:
        ...
        for ind in tqdm(range(0, num_images), ncols=80, desc="locating kps"):
            ...
            for scale in scales:
                ...
                images  = np.zeros((1, 5, inp_height, inp_width), dtype=np.float32)
                ...
                resized_image_xy = cv2.resize(image_xy, (new_width, new_height))
                resized_image, border, offset = crop_image(resized_image, new_center, [inp_height, inp_width])
                resized_image_xy, border, offset = crop_image(resized_image_xy, new_center, [inp_height, inp_width])
                ...
                dets, dets_tl, dets_br, flag = decode_func(nnet, images, K, ae_threshold=ae_threshold, kernel=nms_kernel)
                if not flag:
